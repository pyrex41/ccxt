    ...
    .action(async (file, options) => {
      try {
        ...
        try {
          ...
          const dataPoints = candles.map((candle: any) => ({
    ...
    .action(async (exchange, symbol, timeframe, options) => {
      try {
        ...
        try {
          ...
          if (since) console.log(`  Since:      ${new Date(since).toISOString()}`);
          if (until) console.log(`  Until:      ${new Date(until).toISOString()}`);
 ...
 */
async function exportToCSV(candles: any[], outputFile: string): Promise<void> {
  const lines = [
 ...
 */
async function exportToJSON(candles: any[], outputFile: string): Promise<void> {
 ...
 */
async function importFromCSV(file: string): Promise<any[]> {
  const content = await fs.readFile(file, 'utf-8');
