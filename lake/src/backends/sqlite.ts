...
export class SQLiteBackend extends BaseBackend {
  ...
  private stmtTimeRange: Database.Statement | null = null;
  ...
  async writeMany(points: DataPoint[], options?: WriteOptions): Promise<number> {
    ...
    try {
      ...
      for (let i = 0; i < points.length; i += batchSize) {
        ...
        const transaction = this.db.transaction((items: DataPoint[]) => {
  ...
  async read(query: DataQuery, options?: ReadOptions): Promise<Candle[]> {
    ...
    try {
      ...
      const rows = stmt.all(...params) as Candle[];
  ...
  async findGaps(query: DataQuery, options?: GapDetectionOptions): Promise<Gap[]> {
    ...
    try {
      ...
      const stmt = this.db.prepare(sql);
      const rows = stmt.all(...params) as { current_ts: number; next_ts: number | null }[];
  ...
  async getStats(exchange?: string, symbol?: string): Promise<LakeStats> {
    ...
    try {
      ...
      const exchangeRows = exchangesStmt.all(...params) as { exchange: string }[];
      ...
      const symbolRows = symbolsStmt.all(...params) as { symbol: string }[];
      ...
      const timeframeRows = timeframesStmt.all(...params) as { timeframe: string }[];
      ...
      const timeRow = timeStmt.get(...params) as { oldest: number | null; newest: number | null };
  ...
  async getTimeRange(query: Omit<DataQuery, 'since' | 'until' | 'limit'>): Promise<{
    ...
    try {
      const result = this.stmtTimeRange.get(
        query.exchange,
        query.symbol,
        query.timeframe
      ) as { earliest: number | null; latest: number | null };
