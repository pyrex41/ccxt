...
function createTestCandle(timestamp: number, basePrice: number = 100): Candle {
  return {
    timestamp,
    open: basePrice,
    high: basePrice * 1.1,
...
describe('SQLiteBackend', () => {
  ...
  describe('Connection Tests', () => {
    ...
    it('should throw error when path is not provided', () => {
      expect(() => {
        new SQLiteBackend({ type: 'sqlite' } as any);
  ...
  describe('Write Operations', () => {
    it('should write a single data point', async () => {
      ...
      expect(results[0].open).toBe((point.data as Candle).open);
    });
  ...
  describe('Read Operations', () => {
    ...
    it('should filter by until parameter', async () => {
      ...
      expect(results).toHaveLength(50);
      expect(results[results.length - 1].timestamp).toBeLessThanOrEqual(
        1000000 + 49 * 60000
      );
    });
  ...
  describe('Search Tests (FTS5)', () => {
    ...
    it('should search with prefix mode', async () => {
      ...
      results.forEach(result => {
        expect(
          result.symbol.includes('BTC') ||
            result.exchange.includes('BTC') ||
            result.timeframe.includes('BTC')
        ).toBe(true);
      });
    });
