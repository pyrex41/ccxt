...
import type { DataPoint, Candle, BackendConfig } from '../../src/types.js';
...
// Helper functions
function createTestCandle(timestamp: number, overrides?: Partial<Candle>): Candle {
  return {
    timestamp,
    open: 100,
    high: 110,
...
function createTestDataPoint(
  ...
  timeframe: string,
  timestamp: number,
  candleOverrides?: Partial<Candle>
): DataPoint {
  return {
    timestamp,
    exchange,
...
describe.skipIf(shouldSkip)('TimescaleDBBackend (Integration)', () => {
  let backend: TimescaleDBBackend;
  ...
  describe('Write Operations', () => {
    ...
    it('should write multiple data points in batch', async () => {
      ...
      const points = Array.from({ length: 100 }, (_, i) =>
        createTestDataPoint(testExchange, testSymbol, testTimeframe, now + i * 60000)
      );
    ...
    it('should batch upsert on conflict', async () => {
      ...
      const batch2 = Array.from({ length: 50 }, (_, i) =>
        createTestDataPoint(testExchange, testSymbol, testTimeframe, now + (i + 25) * 60000, { close: 200 })
      );
  ...
  describe('Gap Detection', () => {
    it('should find gaps using window functions', async () => {
      ...
      const points: DataPoint[] = [
        ...
        ...Array.from({ length: 10 }, (_, i) =>
          createTestDataPoint(testExchange, testSymbol, testTimeframe, now + (i + 20) * 60000)
        ),
      ];
    ...
    it('should find multiple gaps', async () => {
      ...
      const points: DataPoint[] = [
        ...
        // Gap 1 (5 minutes)
        ...Array.from({ length: 5 }, (_, i) =>
          createTestDataPoint(testExchange, testSymbol, testTimeframe, now + (i + 10) * 60000)
        ),
        // Gap 2 (5 minutes)
        ...Array.from({ length: 5 }, (_, i) =>
          createTestDataPoint(testExchange, testSymbol, testTimeframe, now + (i + 20) * 60000)
    ...
    it('should respect minGapSize option', async () => {
      ...
      const points: DataPoint[] = [
        ...
        // Small gap (2 minutes)
        ...Array.from({ length: 10 }, (_, i) =>
          createTestDataPoint(testExchange, testSymbol, testTimeframe, now + (i + 12) * 60000)
        ),
      ];
    ...
    it('should work with time range filters', async () => {
      ...
      const points: DataPoint[] = [
        ...
        // Gap 2
        ...Array.from({ length: 10 }, (_, i) =>
          createTestDataPoint(testExchange, testSymbol, testTimeframe, now + (i + 40) * 60000)
        ),
      ];
    ...
    beforeEach(async () => {
      ...
      const points: DataPoint[] = [
        ...Array.from({ length: 100 }, (_, i) =>
          createTestDataPoint('exchange1', 'BTC/USDT', '1m', now + i * 60000)
        ),
        ...Array.from({ length: 50 }, (_, i) =>
          createTestDataPoint('exchange1', 'ETH/USDT', '1m', now + i * 60000)
        ),
        ...Array.from({ length: 75 }, (_, i) =>
          createTestDataPoint('exchange2', 'BTC/USDT', '1h', now + i * 3600000)
        ),
      ];
