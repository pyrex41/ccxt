...
// Helper functions
function createTestCandle(timestamp: number, value: number = 100): Candle {
  return {
    timestamp,
    open: value,
    high: value + 10,
...
function countFiles(dir: string, extension: string = '.parquet'): number {
  let count = 0;
...
describe('ParquetBackend', () => {
  ...
  describe('Write Operations', () => {
    ...
    it('should create partitioned files by date', async () => {
      ...
      const hasPartition1 = checkPartitionStructure(
        testDataPath,
        'binance',
        'BTCUSDT',
        '1m',
      ...
      const hasPartition2 = checkPartitionStructure(
        testDataPath,
        'binance',
        'BTCUSDT',
        '1m',
